...
| `$adhoc` replaced even with 0 filters in `handleApplyAdhocFilters` but **only inside `len>0`** in `handleProcessQueryBatch` (copies diverged) | `resource_handlers.go:396-402` vs `:617-639` | unify after maintainer confirms; Stage A extraction preserves per-handler behavior via a flag |
