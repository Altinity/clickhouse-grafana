 ...
 sum(ProfileEvents['OSCPUVirtualTimeMicroseconds'] as cputime) / 1000000 AS OSCPUVirtualTime,
     /* similar to usertime + system time */
 sum(ProfileEvents['OSIOWaitMicroseconds']) / 1000000 AS OSIOWaitTime,
     /* IO waits, usually disks - that metric is 'orthogonal' to other */
 ...
 sum(result_rows) AS ResultRows,
     ...
     /* same as above but x number of thread */
 sum(ProfileEvents['UserTimeMicroseconds']) / 1000000 AS UserTime,
     /* time when our query was doin some cpu-insense work, creating cpu load */
 sum(ProfileEvents['SystemTimeMicroseconds']) / 1000000 AS SystemTime,
     /* time spend on waiting for some system operations */
 sum(ProfileEvents['NetworkSendElapsedMicroseconds']) / 1000000 AS NetworkSendTime,
     /* check the other side of the network! */
 ...
 WHERE
     ...
     AND $timeFilter
     AND type in (24)-- QueryFinish, ExceptionWhileProcessing

     and normalized_query_hash IN (
)
     query_hash] GROUP BY normalized_query_hash ORDER BY cnt DESC SETTINGS skip_unavailable_shards = 1