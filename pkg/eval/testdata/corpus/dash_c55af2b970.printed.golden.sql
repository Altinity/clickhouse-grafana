 ...
 WHERE
     ...
     AND type <> 1 $conditionalTest( AND hostName() in ($hostname), $hostname) $conditionalTest( AND query_kind in ($query_kind), $query_kind)
 GROUP BY initial_user

 ORDER BY count() DESC SETTINGS skip_unavailable_shards = 1
