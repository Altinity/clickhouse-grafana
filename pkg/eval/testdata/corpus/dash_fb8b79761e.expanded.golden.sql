SELECT t, arrayMap(a -> (a.1, a.2/runningDifference( t/1000 )), groupArr) FROM (SELECT t, groupArray((key, value)) AS groupArr FROM ( SELECT (intDiv(toUInt32(event_time), 30) * 30) * 1000 AS t, 'User.' || toString(from_user) || ', Serv.' || toString(service_name) as key, sum(count) as value FROM
(
    SELECT
