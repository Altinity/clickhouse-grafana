...
func (q *Query) ApplyMacrosAndTimeRangeToQuery() (string, error) {
...
func (q *Query) replace(query string) (string, error) {
	var err error
...
func (q *Query) escapeIdentifier(identifier string) string {
...
func (q *Query) replaceRegexpWithCallBack(re *regexp.Regexp, str string, replacer func([]string) string) string {
	result := ""
	lastIndex := 0
...
func (q *Query) replaceTimeFilters(query string, round int) string {
...
func (q *Query) getFilterSqlForDateTime(columnName string, dateTimeType string) string {
...
func (q *Query) getConvertFn(dateTimeType string) func(string) string {
...
func (q *Query) applyMacros(query string, ast *EvalAST) (string, error) {
...
func (q *Query) contain(ast *EvalAST, field string) bool {
	value, hasValue := ast.Obj[field]
	return hasValue && value != nil && len(value.(*EvalAST).Arr) > 0
}

func (q *Query) _parseMacro(macro string, query string) ([]string, error) {
	var mLen = len(macro)
...
func (q *Query) columns(query string, ast *EvalAST) (string, error) {
...
func (q *Query) _columns(key, value, beforeMacrosQuery, fromQuery string) (string, error) {
...
func (q *Query) rateColumns(query string, ast *EvalAST) (string, error) {
...
func (q *Query) _fromIndex(query, macro string) (int, error) {
...
func (q *Query) rate(query string, ast *EvalAST) (string, error) {
...
func (q *Query) _rate(args []interface{}, beforeMacrosQuery, fromQuery string) (string, error) {
...
func (q *Query) perSecondColumns(query string, ast *EvalAST) (string, error) {
...
func (q *Query) perSecond(query string, ast *EvalAST) (string, error) {
...
func (q *Query) _perSecond(args []interface{}, beforeMacrosQuery, fromQuery string) (string, error) {
...
func (q *Query) _applyTimeFilter(query string) string {
...
func (q *Query) getNaturalTimeSeries(dateTimeType string, from, to int64) string {
...
func (q *Query) getTimeSeries(dateTimeType string) string {
	if dateTimeType == "DATETIME" {
...
func (q *Query) getDateFilter() string {
	return "$dateCol >= toDate($from) AND $dateCol <= toDate($to)"
}

func (q *Query) getDateTimeFilter(dateTimeType string) string {
...
func (q *Query) convertTimestamp(dt time.Time) int64 {
	return dt.UnixMilli() / 1000
}

func (q *Query) round(dt time.Time, round int) time.Time {
	if round == 0 {
		return dt
	}
...
func (q *Query) convertInterval(interval string, intervalFactor int) (int, error) {
	if interval == "" {
		return 0, nil
	}
...
func (q *Query) unescape(query string) (string, error) {
	macros := "$unescape("
...
func (q *Query) betweenBraces(query string) betweenBracesResult {
...
func newScanner(query string) QueryScanner {
	return QueryScanner{
		_sOriginal: query,
		Token:      "",
	}
...
func (s *QueryScanner) raw() string {
	return s._sOriginal
}

func (s *QueryScanner) Next() (bool, error) {
	for {
...
func (s *QueryScanner) expectNext() (bool, error) {
	isNext, err := s.Next()
	if err != nil {
...
func (s *QueryScanner) Format() (string, error) {
	ast, err := s.toAST()
	if err != nil {
		return "", err
	}
...
func (s *QueryScanner) push(argument interface{}) {
...
func (s *QueryScanner) SetRoot(token string) {
	s.RootToken = strings.ToLower(token)
	s.Tree.Obj[s.RootToken] = newEvalAST(false)
	s.expectedNext = true
}

func (s *QueryScanner) isExpectedNext() bool {
	var v = s.expectedNext
	s.expectedNext = false
	return v
}

func (s *QueryScanner) appendToken(argument string) string {
...
func (s *QueryScanner) toAST() (*EvalAST, error) {
	var err error
	s._s = s._sOriginal
...
func (s *QueryScanner) parseJOIN(argument string) (string, error) {
...
func (s *QueryScanner) CheckArrayJOINAndExpectNextOrNext(joinType string) (bool, error) {
...
func (s *QueryScanner) RemoveComments(query string) (string, error) {
