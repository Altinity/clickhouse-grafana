...
export const ColumnSelector = ({datasource, query, selectorType, onChange, onRunQuery}: DateTimeColumnSelectorProps) => {
    ...
    const buildExploreQuery = (type: string): string => {
      switch (type) {
        case 'DATE':
          return 'SELECT name ' +
            'FROM system.columns ' +
            'WHERE database = \'' + query.database + '\' AND ' +
            'table = \'' + query.table + '\' AND ' +
  ...
  return (
    <div className="gf-form">
      <Segment
        options={columnList}
        value={currentValue}
        onChange={(e) => onDateTimeColumnChanged(e!)}
      />
    </div>
  );
};
