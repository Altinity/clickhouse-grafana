...
import {default as SqlQuery2} from '../../v2/src/sql_query'
...
//         expect(SqlQueryMacros.replaceTimeFilters(query, range, 'DATETIME'))
//             .toBe('SELECT * FROM table WHERE column_name >= toDateTime(1545613323) AND column_name <= toDateTime(1546300799)');
//         expect(SqlQueryMacros.replaceTimeFilters(query, range, 'DATETIME64'))
...
//             );
//         expect(SqlQueryMacros.replaceTimeFilters(query64, range, 'DATETIME'))
//             .toBe(
//                 'SELECT * FROM table WHERE ' +
//                 'column_name >= toDateTime64(' + range.from.unix() + '200/1000, 3) AND ' +
//                 'column_name <= toDateTime64(' + range.to.valueOf() + '/1000, 3)'
//             );
//         expect(SqlQueryMacros.replaceTimeFilters(query64, range, 'DATETIME64'))
//             .toBe(
//                 'SELECT * FROM table WHERE ' +
//                 'column_name >= toDateTime64(' + range.from.unix() + '200/1000, 3) AND ' +
//                 'column_name <= toDateTime64(' + range.to.valueOf() + '/1000, 3)'
...
describe("check replace with $columns and concat and ARRAY JOIN", () => {
    ...
    let sql_query = new SqlQuery(target, templateSrv, options);
    let sql_query2 = new SqlQuery2(target, templateSrv, options);
    it("replace with $columns and ARRAY JOIN", () => {
        expect(sql_query2.replace(options, adhocFilters)).toBe(expQuery);
    });
