...
def select_input_query(self, query_name):
    ...
    ActionChains(self.context.driver).double_click(
      locators.sql_editor_input(query_name=query_name, grafana_version=self.context.grafana_version)).click(
      locators.sql_editor_input(query_name=query_name, grafana_version=self.context.grafana_version)).perform()
...
def change_expression_name(self, expression_name, new_expression_name):
    ...
    locators.expression_name_textfield(expression_name=expression_name, grafana_version=self.context.grafana_version).send_keys(
      new_expression_name)
...
def enter_expression_operation(self, expression_name, operation_type):
    ...
    locators.expression_operation_dropdown(expression_name=expression_name, grafana_version=self.context.grafana_version).send_keys(
      operation_type)
    locators.expression_operation_dropdown(expression_name=expression_name, grafana_version=self.context.grafana_version).send_keys(
      Keys.ENTER)
