...
@TestScenario
def gh_play_check(self):
    """Check that gh-play data source works correctly."""

    with When("I go to clickhouse dashboard"):
        dashboards.open_dashboard(dashboard_name="dashboard-gh-play")


    with Then("I check gh-play datasource works correctly"):
        for attempt in retries(delay=5, timeout=50):
            with attempt:
                with delay():
                    with Then("I take screenshot of Coverage CI checks per day panel"):
                        dashboard.take_screenshot_for_panel(panel_name="Coverage CI checks per day", screenshot_name="gh-play_panel")

                with delay():
                    with Then("I check graph contains data"):
                        assert actions.check_screenshot_contains_green(screenshot_name="gh-play_panel") is True, error()
