...
def time_shift(self):
    ...
    try:
        ...
        with Then("I check reformatted query contains properly defined time from and time to"):
            with delay():
                # both timestamps are relative to "now", which advances between
                # the two reads, so allow up to 10 minutes of test wall-clock
                assert time_to_before_time_shift - sql_editor.get_time_to_in_seconds(query_name='A') > 3*60*60 - 600, error()
                assert time_from_before_time_shift - sql_editor.get_time_from_in_seconds(query_name='A') > 3*60*60 - 600, error()
...
def feature(self):
    ...
    with Given("I define dashboard name for tests"):
        dashboard_name = define("dashboard_name", "query_options")
